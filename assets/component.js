...
class CartItemsComponent extends Component {
  ...
  updateQuantity(config) {
    ...
    fetch(`${Theme.routes.cart_change_url}`, fetchConfig('json', { body }))
      ...
      .then((responseText) => {
        ...
        // Sync the dialog's empty state class after morphing
  ...
  #handleCartUpdate = (event) => {
    ...
    if (cartItemsHtml) {
      ...
      // After morphing, sync the dialog's cart-drawer--empty class with the actual state
      // The morph updates the content but the dialog class may need manual sync
      this.#syncDialogEmptyState();
    } else {
      sectionRenderer.renderSection(this.sectionId, { cache: false });
    }
  ...
  #syncDialogEmptyState() {
    ...
    // Check if cart has items by looking for the cart form or cart items
    const hasItems = this.querySelector('.cart-drawer__items') !== null;
